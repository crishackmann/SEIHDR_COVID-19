...
number_reported = c(52,75,86,85,85,97,88,93,94,100,104,97,102,110,102,105,108,108,110,109,125,129,133,127,135,136,139,165,158,173,179,175,187,184,206,212,215,214,241,247,274,275,287,311,313)
...
loglikelihood_fun = function(parameters, dat) { 
  ...
  LL = -sum(dpois(x = dat$number_reported, lambda = 1 * output$H[output$time %in% dat$time], log = TRUE))
  print(LL)
  return(LL) 
}
...
# PLot the model fit
base_1 = ggplot() +
  geom_line(data = output, aes(x = time, y = H), colour = '#119299') +
  geom_point(data = reported_data, aes(x = time, y = number_reported, colour = "Hospitalizados reais")) + 
  xlab("Data") +                                              
  ylab("Número de Hospitalizados") +                                 
  labs(title = "Modelo ajustado à curva epidêmica", colour = "") +
